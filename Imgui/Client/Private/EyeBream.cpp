...
CEyeBream::CEyeBream(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{

}

CEyeBream::CEyeBream(const CEyeBream & rhs)
	: CGameObject(rhs)
{
...
HRESULT CEyeBream::Initialize_Prototype()
{
...
HRESULT CEyeBream::Initialize(void * pArg)
{
	...
	m_vOriScale = _float3(0.05f, 0.1f, 0.05f);
...
void CEyeBream::Tick(_float fTimeDelta)
{
...
void CEyeBream::LateTick(_float fTimeDelta)
{
...
HRESULT CEyeBream::Ready_Components()
{
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("EyeAttack"), TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
CEyeBream * CEyeBream::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CEyeBream*		pInstance = new CEyeBream(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CEyeBream"));
...
CGameObject * CEyeBream::Clone(void * pArg)
{
	CEyeBream*		pInstance = new CEyeBream(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : CEyeBream"));
		Safe_Release(pInstance);
	}
...
void CEyeBream::Tick_ImGui(_float fTimeDelta)
{
