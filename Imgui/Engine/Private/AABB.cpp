...
_bool CAABB::Collision(CCollider * pTargetCollider)
{
	...
	case CCollider::TYPE_AABB:
		bIsColl = m_pAABB->Intersects(((CAABB*)pTargetCollider)->Get_Collider());
		break;

	case CCollider::TYPE_OBB:
		bIsColl = m_pAABB->Intersects(((COBB*)pTargetCollider)->Get_Collider());
		break;

	case CCollider::TYPE_SPHERE:
		bIsColl = m_pAABB->Intersects(((CSphere*)pTargetCollider)->Get_Collider());
		break;
	}
...
void CAABB::Edit_Col(COLLIDERDESC Desc)
{

	CCollider::Edit_Col(Desc);

	m_pOriginal_AABB->Center = m_ColliderDesc.vCenter;
	m_pOriginal_AABB->Extents = _float3(m_ColliderDesc.vSize.x * 0.5f, m_ColliderDesc.vSize.y * 0.5f, m_ColliderDesc.vSize.z * 0.5f);
