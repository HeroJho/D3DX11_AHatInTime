...
HRESULT CWind_Spiral::Initialize(void * pArg)
{
	...
	WINDSPIRALDESC* pDesc = (WINDSPIRALDESC*)pArg;
	


	m_pTransformCom->Rotation(XMVectorSet(0.f, 1.f, 0.f, 0.f), XMConvertToRadians(pDesc->fAngle));
	m_pTransformCom->Set_Scale(XMLoadFloat3(&pDesc->vScale));
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSetW(XMLoadFloat3(&pDesc->vParPos) + XMLoadFloat3(&pDesc->vLocalPos), 1.f));

	m_fOriY = pDesc->vScale.y;

	m_fMaxScale = pDesc->fMaxScale;
	m_fScaleSpeed = pDesc->fScaleSpeed;
...
void CWind_Spiral::Tick(_float fTimeDelta)
{
	...
	m_pTransformCom->Set_Scale(XMVectorSet(vScale.x, m_fOriY * (1.f - m_fRatio), vScale.x, 1.f));
	if (1.f < m_fRatio)
		Set_Dead(true);
...
HRESULT CWind_Spiral::Render()
{
	...
	for (_uint i = 0; i < iNumMeshes; ++i)
	{
		...
		if (FAILED(m_pModelCom->Render(m_pShaderCom, i, 15)))
			return E_FAIL;
	}
