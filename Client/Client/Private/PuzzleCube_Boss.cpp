#include "stdafx.h"
#include "..\Public\PuzzleCube_Boss.h"
...
CPuzzleCube_Boss::CPuzzleCube_Boss(const CPuzzleCube_Boss & rhs)
	: CGameObject(rhs)
{
}
...
HRESULT CPuzzleCube_Boss::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CPuzzleCube_Boss::Initialize(void * pArg)
{
	if (nullptr == pArg)
		return E_FAIL;
...
void CPuzzleCube_Boss::Tick(_float fTimeDelta)
{
	...
	switch (m_eState)
	{
	case Client::CPuzzleCube_Boss::STATE_IN:
		Tick_In(fTimeDelta);
		break;
	case Client::CPuzzleCube_Boss::STATE_OUT:
		Tick_Out(fTimeDelta);
		break;
	}
...
void CPuzzleCube_Boss::Tick_In(_float fTimeDelta)
{

}

void CPuzzleCube_Boss::Tick_Out(_float fTimeDelta)
{
...
void CPuzzleCube_Boss::LateTick(_float fTimeDelta)
{
	if (!m_bInitDrop)
		return;
...
void CPuzzleCube_Boss::OnCollision(CCollider::OTHERTOMECOLDESC Desc)
{
...
HRESULT CPuzzleCube_Boss::Ready_Components()
{
	/* For.Com_Transform */
...
CPuzzleCube_Boss * CPuzzleCube_Boss::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CPuzzleCube_Boss*		pInstance = new CPuzzleCube_Boss(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CPuzzleCube_Boss"));
...
CGameObject * CPuzzleCube_Boss::Clone(void * pArg)
{
	CPuzzleCube_Boss*		pInstance = new CPuzzleCube_Boss(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : CPuzzleCube_Boss"));
		Safe_Release(pInstance);
	}
